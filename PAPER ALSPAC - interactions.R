...
#server-side object:
ds.rm("impFit.DS", datasources = connections[3])
#locally:
rm("impFit.DS")
...
impFit.DS <- ds.glmSLMA(formula = 'medall ~ bin_edu_m * smokingduringpreg + agebirth_m_y + asthma_m + asthma_bf',
                        family = "poisson", dataName = "Dmedall11", newobj ="impFit.DS",
                        combine.with.metafor = FALSE, datasources = connections[3])
#see output
impFit.DS
...
ds.rm("expData", datasources = connections[3])
#locally:
rm("expData")

ds.neImpute(object = 'impFit.DS', newobj = 'expData', datasources = connections[3])

#check the names of the two imputed counterfactual outcomes
ds.colnames('expData', datasources = connections[3])
#bin_edu_m0 & bin_edu_m1


########################## STEP 3: Fit natural effect model to the imputed dataset #######################
ds.rm("neMod.DS", datasources = connections[3])
#locally:
rm("neMod.DS")
...
neMod.DS
#### INTERACTION TERM INSIGNIIFICANT: bin_edu_m02:bin_edu_m12 p = 
...
######+SLMA:
#added combine.with.metafor = FALSE, the option for not combining with metafor
impFit.DS <- ds.glmSLMA(formula = 'poisson_medall ~ bin_edu_m * adverse_rep_outcomes + agebirth_m_y + asthma_m + asthma_bf',
                        family = "poisson", dataName = "Dmedall11", newobj ="impFit.DS",
                        combine.with.metafor = FALSE, datasources = connections[3])
#see output
impFit.DS
...
######+SLMA:
#added combine.with.metafor = FALSE, the option for not combining with metafor
impFit.DS <- ds.glmSLMA(formula = 'poisson_medall ~ bin_edu_m * breastfedcat + agebirth_m_y + asthma_m + asthma_bf',
                        family = "poisson", dataName = "Dmedall11", newobj ="impFit.DS",
                        combine.with.metafor = FALSE, datasources = connections[3])
#see output
impFit.DS
...
neMod.DS
#### INTERACTION TERM SIGNIIFICANT!!!!!: bin_edu_m02:bin_edu_m12 p = 
...
######+SLMA:
#added combine.with.metafor = FALSE, the option for not combining with metafor
impFit.DS <- ds.glmSLMA(formula = 'poisson_medall ~ bin_edu_m * passivesmoke2y + agebirth_m_y + asthma_m + asthma_bf',
                        family = "poisson", dataName = "Dmedall11", newobj ="impFit.DS",
                        combine.with.metafor = FALSE, datasources = connections[3])
#see output
impFit.DS
